...
export function CheckoutDialog({ open, onOpenChange, plano, isAnual }: CheckoutDialogProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    // Set flags to prevent redirect during checkout and pass plan info
    sessionStorage.setItem("checkout_in_progress", "true");
    sessionStorage.setItem("checkout_plano", plano.nome);
    sessionStorage.setItem("checkout_isAnual", String(isAnual));
    
    const redirectUrl = `${window.location.origin}/prospeccao?checkout=google_success&plano=${encodeURIComponent(plano.nome)}&isAnual=${isAnual}`;
