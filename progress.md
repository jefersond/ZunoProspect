...
### Iteração 18: Drift Git/Produção e Fallback de IA para Rate Limit Sustentado (26/07/2026)
- [x] Reportado pelo usuário: "Refinar com IA" falhando repetidamente com toast "O serviço de IA está temporariamente ocupado", com múltiplos 429 no console do navegador.
- [x] Identificado, via `mcp__Supabase__get_edge_function`, que a Edge Function `analisar-lead-ia` publicada em produção (projeto `ihtltqxxlvbsxbiacbpr`) usa uma arquitetura modular (`_shared/refine-observability.ts`, `credit-policy.ts`, `refine-persistence.ts`, `refine-quality.ts`, `refine-retry-policy.ts`) inteiramente ausente do repositório Git, que ainda tinha uma versão antiga e monolítica do `index.ts`.
- [x] Confirmada a causa raiz via `mcp__Supabase__get_logs` (`edge-function`): a função retorna HTTP 429 diretamente ao cliente quando os 2 modelos da cascata (`gemini-2.5-pro`, `gemini-2.5-flash`) esgotam o orçamento de 45s sob rate limit sustentado do Gemini, sem nenhum fallback local, apesar de o motor de fallback por foco (`generateMockAnalise`/`buildFallbackProspectingPlan`) já existir no arquivo para outro caso de uso (falha de qualidade da resposta da IA).
- [x] Sincronizados os 7 arquivos reais de produção para o Git (`supabase/functions/analisar-lead-ia/index.ts`, `focusBehavior.ts`, e os 5 novos módulos em `supabase/functions/_shared/`).
- [x] Corrigido `index.ts`: chamada ao provedor + validação de qualidade agora envoltas em `try/catch`; qualquer falha do provedor (exceto falta de configuração da API key) aciona `generateMockAnalise` como fallback local, mantendo o fluxo normal de persistência, consumo de crédito e resposta de sucesso (`qualityResult.fallbackUsed = true`, motivo propagado para `ai_fallback_reason`).
- [x] Validada a ausência de erros de sintaxe introduzidos via checagem `tsc --noEmit --noResolve` no arquivo modificado.
- [ ] **Pendente:** deploy real da Edge Function corrigida no Supabase remoto — arquivo grande demais (~200KB) para reenvio seguro via MCP nesta sessão; requer `supabase functions deploy analisar-lead-ia` via CLI/CI vinculado ao projeto.
