-- Allow every authenticated plan to correct only leads owned by that user.
-- The Edge Function remains the only caller with the encryption key and service role.

CREATE OR REPLACE FUNCTION public.update_owned_lead_core_fields(
...
BEGIN
  ...
  IF p_edited_by IS NULL THEN
    RAISE EXCEPTION 'Authenticated editor is required';
  END IF;
  ...
  WHERE l.id = p_lead_id
    AND (l.user_id = p_edited_by OR public.is_admin(p_edited_by));

  IF v_before IS NULL THEN
    RAISE EXCEPTION 'Lead not found or does not belong to this user';
  END IF;
  ...
  SET
    ...
    updated_at = NOW()
  WHERE id = p_lead_id
    AND (user_id = p_edited_by OR public.is_admin(p_edited_by))
  RETURNING id INTO v_updated_id;

  IF v_updated_id IS NULL THEN
    RAISE EXCEPTION 'Lead update was not authorized';
  END IF;
...
REVOKE ALL ON FUNCTION public.update_owned_lead_core_fields(
  TEXT, UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT
) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.update_owned_lead_core_fields(
  TEXT, UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT
) TO service_role;

COMMENT ON FUNCTION public.update_owned_lead_core_fields(
  TEXT, UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT
) IS 'Service-role lead correction with owner-or-admin authorization and encrypted sensitive fields.';
