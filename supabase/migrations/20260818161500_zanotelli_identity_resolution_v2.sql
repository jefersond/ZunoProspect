...
as $$
  select case
    when lower(trim(coalesce(value, ''))) ~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$'
...
as $$
  select case
    when public.zanotelli_normalize_email(value) is not null
      then encode(digest('zanotelli-email:v1:' || public.zanotelli_normalize_email(value), 'sha256'), 'hex')
    else null
  end
$$;
...
as $$
  select case
    when public.zanotelli_normalize_domain(value) ~ '^[a-z0-9][a-z0-9.-]{1,251}\.[a-z]{2,63}$'
      then encode(digest('zanotelli-domain:v1:' || public.zanotelli_normalize_domain(value), 'sha256'), 'hex')
    else null
  end
$$;
...
as $$
  select case
    when char_length(public.zanotelli_normalize_identity_text(value)) between 2 and 180
      then encode(digest('zanotelli-company:v1:' || public.zanotelli_normalize_identity_text(value), 'sha256'), 'hex')
    else null
  end
$$;
...
as $$
  select case
    when char_length(public.zanotelli_normalize_identity_text(value)) between 2 and 180
      then encode(digest('zanotelli-responsible:v1:' || public.zanotelli_normalize_identity_text(value), 'sha256'), 'hex')
    else null
  end
$$;
...
begin
  ...
  if matched_by is null and phone_hash ~ '^[0-9a-f]{64}$' then
    ...
    if candidate_count = 1 then
      ...
      return jsonb_build_object('status', 'ambiguous', 'candidate_count', candidate_count, 'matched_by', 'phone');
    end if;
  end if;
  ...
  if matched_by is null and email_hash ~ '^[0-9a-f]{64}$' then
    ...
    join public.zanotelli_context_scope_users scope
      on scope.user_id = lead.user_id
     and scope.active = true
    where public.zanotelli_email_lookup_hash(lead.email) = email_hash
       or public.zanotelli_email_lookup_hash(lead.cnpj_email) = email_hash;

    if candidate_count = 1 then
      matched_by := 'email';
    elsif candidate_count > 1 then
      return jsonb_build_object('status', 'ambiguous', 'candidate_count', candidate_count, 'matched_by', 'email');
    end if;
  end if;
  ...
  if matched_by is null and domain_hash ~ '^[0-9a-f]{64}$' then
    ...
    if candidate_count = 1 then
      ...
      return jsonb_build_object('status', 'ambiguous', 'candidate_count', candidate_count, 'matched_by', 'domain');
    end if;
  end if;
  ...
  if matched_by is null and company_hash ~ '^[0-9a-f]{64}$' then
    if responsible_hash ~ '^[0-9a-f]{64}$' then
      ...
      join public.zanotelli_context_scope_users scope
       ...
       and scope.active = true
      where (
        public.zanotelli_company_lookup_hash(lead.nome) = company_hash
        or public.zanotelli_company_lookup_hash(lead.company_name) = company_hash
      )
      and public.zanotelli_responsible_lookup_hash(lead.nome_responsavel) = responsible_hash;

      if candidate_count = 1 then
        matched_by := 'company_responsible';
      elsif candidate_count > 1 then
        return jsonb_build_object('status', 'ambiguous', 'candidate_count', candidate_count, 'matched_by', 'company_responsible');
      end if;
    end if;
    ...
    if matched_by is null then
      ...
      join public.zanotelli_context_scope_users scope
        on scope.user_id = lead.user_id
       and scope.active = true
      where public.zanotelli_company_lookup_hash(lead.nome) = company_hash
         or public.zanotelli_company_lookup_hash(lead.company_name) = company_hash;

      if candidate_count = 1 then
        matched_by := 'company';
      elsif candidate_count > 1 then
        return jsonb_build_object('status', 'ambiguous', 'candidate_count', candidate_count, 'matched_by', 'company');
      end if;
    end if;
  end if;

  if matched_by is null or selected_lead_id is null then
