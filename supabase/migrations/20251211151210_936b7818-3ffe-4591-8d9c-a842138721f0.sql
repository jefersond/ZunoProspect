-- Atualiza função para aceitar user_id como parâmetro opcional
-- Isso permite que service role passe o user_id explicitamente

DROP FUNCTION IF EXISTS public.get_lead_decrypted_by_id(uuid);

CREATE OR REPLACE FUNCTION public.get_lead_decrypted_by_id(p_lead_id uuid, p_user_id uuid DEFAULT NULL)
RETURNS TABLE (
  id uuid,
