...
const oldFetchCall = `    // Usa fetchWithRetry para lidar com rate limits (429)
...
const newFetchCall = `    // Usa fetchWithRetry para lidar com rate limits (429)
    const response = await fetchWithRetry(
      \`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=\${apiKey}\`,
      {
        method: "POST",
...
console.log("Iniciando substituições determinísticas no arquivo da Edge Function via Node.js...");
...
console.log("Substituições concluídas com absoluto sucesso via Node.js!");
